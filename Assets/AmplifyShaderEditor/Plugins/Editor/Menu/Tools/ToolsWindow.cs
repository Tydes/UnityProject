...
namespace AmplifyShaderEditor
{
	...
	public sealed class ToolsWindow : MenuParent
	{
		...
		public ToolsWindow() : base( 0, 0, 0, 64, "Tools", MenuAnchor.TOP_LEFT, MenuAutoSize.NONE )
		{
			...
			ToolsMenuButton updateButton = new ToolsMenuButton( ToolButtonType.Update, 0, 0, -1, -1, "UI/Buttons/UpdateOutdated", string.Empty, "Create and apply shader to material.", 5 );
			updateButton.ToolButtonPressedEvt += OnButtonPressedEvent;
			updateButton.AddState( "UI/Buttons/UpdateOFF" );
			updateButton.AddState( "UI/Buttons/UpdateUpToDated" );
			m_list[ ( int ) ToolButtonType.Update ] = updateButton;

			ToolsMenuButton liveButton = new ToolsMenuButton( ToolButtonType.Live, 0, 0, -1, -1, "UI/Buttons/LiveOFF", string.Empty, "Automatically saves shader when canvas is changed.", 50 );
			liveButton.ToolButtonPressedEvt += OnButtonPressedEvent;
			liveButton.AddState( "UI/Buttons/LiveON" );
			liveButton.AddState( "UI/Buttons/LivePending" );
			m_list[ ( int ) ToolButtonType.Live ] = liveButton;

			ToolsMenuButton cleanUnusedNodesButton = new ToolsMenuButton( ToolButtonType.CleanUnusedNodes, 0, 0, -1, -1, "UI/Buttons/CleanupOFF", string.Empty, "Remove all nodes not connected to the master node.", 77 );
			cleanUnusedNodesButton.ToolButtonPressedEvt += OnButtonPressedEvent;
			cleanUnusedNodesButton.AddState( "UI/Buttons/CleanupON" );
			m_list[ ( int ) ToolButtonType.CleanUnusedNodes ] = cleanUnusedNodesButton;


			ToolsMenuButton openSourceCodeButton = new ToolsMenuButton( ToolButtonType.OpenSourceCode, 0, 0, -1, -1, "UI/Buttons/OpenSourceCodeOFF", string.Empty, "Open shader file in your default shader editor.", 110 );
			openSourceCodeButton.ToolButtonPressedEvt += OnButtonPressedEvent;
			openSourceCodeButton.AddState( "UI/Buttons/OpenSourceCodeON" );
			...
			m_focusOnMasterNodeButton = new ToolsMenuButton( ToolButtonType.FocusOnMasterNode, 0, 0, -1, -1, "UI/Buttons/FocusNode", string.Empty, "Focus on active master node." );
			m_focusOnMasterNodeButton.ToolButtonPressedEvt += OnButtonPressedEvent;

			m_focusOnSelectionButton = new ToolsMenuButton( ToolButtonType.FocusOnSelection, 0, 0, -1, -1, "UI/Buttons/FitView", string.Empty, "Focus on selection or fit to screen if none selected." );
